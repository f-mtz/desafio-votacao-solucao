...
spring.datasource.username=VOTACAO_DB
spring.datasource.password=StrongPwd123
...
# Base URL para geração de links e Swagger server
...
1. Crie um Agenda (pauta), que representa o evento de votação: POST /api/v1/agendas
2. Crie uma sessão (janela de votação): POST /api/v1/session/abrir-sessao
3. Realize o voto: POST /api/votes/votar

```sql
-- Criando banco via DBeaver ou SQL Server
-- Execute como SYS ou usuário com privilégios
CREATE USER VOTACAO_DB IDENTIFIED BY "StrongPwd123"
  DEFAULT TABLESPACE USERS
  QUOTA UNLIMITED ON USERS;

GRANT CREATE SESSION, CREATE TABLE, CREATE SEQUENCE, CREATE VIEW, CREATE TRIGGER, CREATE PROCEDURE TO VOTACAO_DB;

-- (Opcional) Trabalhar no schema recém-criado sem trocar de conexão:
-- ALTER SESSION SET CURRENT_SCHEMA=VOTACAO_DB;
...
CREATE TABLE voting_session (
  ...
  end_time   TIMESTAMP,
  CONSTRAINT pk_voting_session PRIMARY KEY (id),
  CONSTRAINT uq_voting_session_agenda UNIQUE (agenda_id),
...
CREATE TABLE vote (
  ...
  associate_id  VARCHAR2(255) NOT NULL,
  vote_type     VARCHAR2(10)
    CHECK (vote_type IN ('SIM','NAO')),
  CONSTRAINT pk_vote PRIMARY KEY (id),
  CONSTRAINT uq_vote_session_associate UNIQUE (session_id, associate_id),
